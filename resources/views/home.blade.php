      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $group->name }}</td>
            <div><td><a class="btn btn-primary m-3" href="{{ action('User\GroupController@talk', ['id' => $group->id] ) }}">トーク</a></td></div>
            <form action="{{ action('User\GroupController@withdraw')}}" method="post">
              @csrf
              <td><button type="submit" name="id" value="{{ $group->id }}" class="m-2 btn btn-danger">退会</button></td>
              {{-- {{ csrf_field() }} --}}
            </form>
          </tr>
