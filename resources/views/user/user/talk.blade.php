...
@section('nameOf')
  <p class="h3 mx-auto">{{ $user->name }}</p>
@endsection
...
function get_messageU() {
  $.ajax({
    //url: "/result/ajax/",
    url: "{{ action('User\UserController@getMessageU',['id'=>$user->id]) }}",
    dataType: "json",
    success: data => {
      $("#message-data")
  ...
  setTimeout("get_messageU()",10000);
}
</script>
@endsection

@section('form')
    <form id="message_form" action="{{ action('User\UserController@sendC') }}" name="message_form" class="form-inline d-flex method="post" justify-content-between" enctype="multipart/form-data">
        {{ csrf_field() }}
